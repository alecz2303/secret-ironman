...
function calcula(y){
    ...
    var row = Number(matches[1]);
        return $.ajax({
            url: '../invSalesItems/itemAll',
            data:{item_id:item_id},
            type:'POST',
            success: function(data) {
            var respuesta = JSON.parse(data);
            document.getElementById('InvSalesItems_'+row+'_description').value=respuesta.description;
            document.getElementById('InvSalesItems_'+row+'_item_unit_price').value=respuesta.unit_price;
            document.getElementById('InvSalesItems_'+row+'_item_cost_price').value=respuesta.cost_price;
            document.getElementById('InvSalesItems_'+row+'_qty_exist').value=respuesta.qty;
            document.getElementById('InvSalesItems_'+row+'_total').value=respuesta.unit_price;
            total();
            }
        });
...
function suma(y){
    ...
    if(document.getElementById('InvSalesItems_'+row+'_discount_percent').value>0){
        document.getElementById('InvSalesItems_'+row+'_total').value=(document.getElementById('InvSalesItems_'+row+'_item_unit_price').value * document.getElementById('InvSalesItems_'+row+'_qty_purchased').value)-((document.getElementById('InvSalesItems_'+row+'_discount_percent').value*document.getElementById('InvSalesItems_'+row+'_total').value)/100);
    }else{
        document.getElementById('InvSalesItems_'+row+'_total').value=document.getElementById('InvSalesItems_'+row+'_item_unit_price').value * document.getElementById('InvSalesItems_'+row+'_qty_purchased').value;
    }    
    total();
}
...
function total(){
    ...
    document.getElementById('InvFullNameCustomer_grand').value = '$ '+tot.formatMoney(2, '.', ',');
}
...
function validateForm(){
    ...
    var arrExist = document.getElementsByClassName('inputQty_exist');
    ...
    for(var i=0;i<arr.length;i++){
        ...
        if(parseInt(arrQty[i].value)>parseInt(arrExist[i].value)){
            totExist ++;
            arrExist[i].style.backgroundColor="rgba(153,51,51,0.5)";
            arrExist[i].style.border="solid 1px #993333";
            arrExist[i].style.color="#FFFFFF";
        }else{
            arrExist[i].style.backgroundColor="rgba(111,174,127,0.5)";
            arrExist[i].style.border="solid 1px #6fae7f";
            arrExist[i].style.color="#000000";
        }
    ...
    if(totExist>0){
        $(".item_qty_exist").val(-10000);
        //return false;    
    }else{
        $(".item_qty_exist").val(1000000);
    }
...
function valida(y){
    ...
    var row = Number(matches[1]);
        return $.ajax({
            url: '../invSalesItems/serialNumber',
            ...
            if(respuesta.serialnumber=='ACCEPTED' || respuesta.serialnumber==''){
                ...
                alert('El numero de serie ya existe en la salida # '+ respuesta.salesid);
                y.focus();
            }
            total();
            }
