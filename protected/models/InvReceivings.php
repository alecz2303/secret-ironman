<?php

/**
 * This is the model class for table "inv_receivings".
 *
 * The followings are the available columns in table 'inv_receivings':
 * @property integer $id
 * @property string $receiving_time
 * @property integer $suplier_id
 * @property string $user
 * @property string $comment
 *
 * The followings are the available model relations:
 * @property InvSuppliers $suplier
 * @property InvReceivingsItems[] $invReceivingsItems
 */
class InvReceivings extends CActiveRecord
{
    public $company_name_search;
    public $grand;
    public $item_id;
    public $description;
    public $qty_purchased;
    public $item_unit_price;
    public $item_cost_price;
    
	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'inv_receivings';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
                        array('suplier_id, item_id, description, qty_purchased, item_cost_price, item_unit_price','required'),
			array('suplier_id', 'numerical', 'integerOnly'=>true),
			array('user', 'length', 'max'=>50),
			array('comment', 'length', 'max'=>45),
			array('receiving_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, company_name_search, receiving_time, suplier_id, user, comment', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
			'suplier' => array(self::BELONGS_TO, 'InvSuppliers', 'suplier_id'),
			'invReceivingsItems' => array(self::HAS_MANY, 'InvReceivingsItems', 'receivings_id'),
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'receiving_time' => 'Fecha de Recepción',
			'suplier_id' => 'Proveedor',
			'user' => 'Usuario',
			'comment' => 'Comentario',
            'company_name_search' => 'Nombre de la Compañia',
            'grand'=>'Gran Total',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 *
	 * Typical usecase:
	 * - Initialize the model fields with values from filter form.
	 * - Execute this method to get CActiveDataProvider instance which will filter
	 * models according to data in model fields.
	 * - Pass data provider to CGridView, CListView or any similar widget.
	 *
	 * @return CActiveDataProvider the data provider that can return the models
	 * based on the search/filter conditions.
	 */
	public function search()
	{
		// @todo Please modify the following code to remove attributes that should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('receiving_time',$this->receiving_time,true);
		$criteria->compare('suplier_id',$this->suplier_id);
		$criteria->compare('user',$this->user,true);
		$criteria->compare('comment',$this->comment,true);
                $criteria->with = array('suplier');
                $criteria->compare('suplier.company_name',$this->company_name_search,true);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
                        'sort'=>array(
                            'attributes'=>array(
                                'company_name_search'=>array(
                                    'asc'=>'suplier.company_name',
                                    'desc'=>'suplier.company_name DESC',
                                ),
                                '*',
                            ),
                        ),
		));
	}
        
        public function getMenuSuppliers(){
		$suppliers=  InvSuppliers::model()->findAll(array('order'=>'company_name'));
		return CHtml::listData($suppliers,"id","company_name");
	}
        
        /**
	 * Returns the static model of the specified AR class.
	 * Please note that you should have this exact method in all your CActiveRecord descendants!
	 * @param string $className active record class name.
	 * @return InvReceivings the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}
}
