<?php
$this->pageTitle=Yii::app()->name . ' - Busqueda de Respaldo de Escrituras en Libros Protocolo Especial';
$this->breadcrumbs=array(
    'Busqueda de Respaldo de Escrituras en Libros Protocolo Especial',
);

?>

<h1>B&uacute;squeda de documentos Protocolo Especial</h1>
...
<?php 
    if(isset($_POST['BusquedaForm'])){
		...
		if ( ! empty( $searchstr ) ) {
            ...
            $cmdstr = "find '/mnt/antiguo/Protocolo Especial/Respaldo de Libros' -type f -name '*.DO*' -print0 | xargs -0 grep -io '$searchstr'";
			$cmdstr1 = "find '/mnt/antiguo/Protocolo Especial/Respaldo de Libros' -type f -name '*.do*' -print0 | xargs -0 grep -io '$searchstr'";
			...
			// So, we use split() to split the data
                            ...
                            list( $fname, $fline ) = explode( "Binary file /mnt/antiguo/Protocolo Especial/Respaldo de Libros", $buffer);
			...
			// So, we use split() to split the data
                            ...
                            list( $fname1, $fline1 ) = explode( "Binary file /mnt/antiguo/Protocolo Especial/Respaldo de Libros", $buffer);
                            // we take only the first hit per file
                            //if ( !defined( $myresult[$fname] ) ) {
                            //$myresult[$fname] = $fline;
                            //}
