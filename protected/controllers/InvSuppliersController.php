...
class InvSuppliersController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				...
				//'users'=>array('@'),
                                'expression'=> array('InvSuppliersController','allowOnlyGroup'),
			),
	...
	{
		$model=new InvSuppliers;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['InvSuppliers']))
		{
			$model->attributes=$_POST['InvSuppliers'];
                        ...
                        if($people->save()){
                            ...
                            $model->deleted = 0;
                            if($model->save()){
                                    Yii::app()->user->setFlash('success','Se ha creado el Proveedor <b>'.$model->company_name.'</b>.');
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['InvSuppliers']))
		{
			$model->attributes=$_POST['InvSuppliers'];
                        ...
                        if($people->save()){
                            if($model->save())
                                    Yii::app()->user->setFlash('info','Se han actualizado los datos del Proveedor <b>'.$model->company_name.'</b>.');
                                    $this->redirect(array('view','id'=>$model->id));
                        }
		}
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			//$this->loadModel($id)->delete();
                        ...
                        Yii::app()->user->setFlash('danger','Se ha eliminado el Proveedor <b>'.$model->company_name.'</b>.');
	...
	{
		$dataProvider=new CActiveDataProvider('InvSuppliers');
	...
	{
		$model=new InvSuppliers('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['InvSuppliers']))
			$model->attributes=$_GET['InvSuppliers'];
	...
	{
		$model=InvSuppliers::model()->findByPk($id);
		if($model===null)
        ...
        public function actionDelMunByEst(){
            ...
            $list=Codigos::model()->findAllBySql("SELECT DISTINCT c_mnpio,D_mnpio  FROM codigos WHERE c_estado='".$_POST["InvSuppliers"]["estado"]."' ORDER BY D_mnpio");
        ...
        public function actionColoniaByDelMun(){
                $list=Codigos::model()->findAllBySql("SELECT DISTINCT id_asenta_cpcons,d_asenta,d_codigo  FROM codigos WHERE c_estado='".$_POST["InvSuppliers"]["estado"]."' AND c_mnpio='".$_POST["InvSuppliers"]["mnpio"]."' GROUP BY id_asenta_cpcons, d_asenta ORDER BY d_asenta");
                echo"<option value=\"\">--</option>";
                foreach($list as $data)
        ...
        public function actionCodigoByColonia(){
                $list=Codigos::model()->findAllBySql("SELECT id,d_codigo FROM codigos WHERE c_estado=".$_POST["InvSuppliers"]["estado"]." AND c_mnpio=".$_POST["InvSuppliers"]["mnpio"]." AND id_asenta_cpcons=".$_POST["InvSuppliers"]["col"]." GROUP BY id_asenta_cpcons, d_asenta ORDER BY d_asenta");
                foreach($list as $data)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='inv-suppliers-form')
		{
